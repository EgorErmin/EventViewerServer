package eventrev.net.service;

import eventrev.net.model.ResponseData;
import eventrev.net.model.ResponseType;

public class ValidationService {
    private Boolean email(String email){
        return email.matches("(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])");
    }

    private Boolean password(String password){
        return password.length() >= 4 && password.length() <= 10;
    }

    public ResponseData register(String email, String password){
        if(!this.email(email)){
            return ResponseData.type(ResponseType.INVALID_EMAIL);
        }
        if(!this.password(password)){
            return ResponseData.type(ResponseType.INVALID_PASSWORD);
        }
        return null;
    }
}
